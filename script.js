...
function handleSearchInput(input) {
  // 只記錄使用者正在輸入的內容，不重新渲染畫面，避免注音/拼音選字被中斷。
  searchDraftKeyword = input.value;
}
...
function runSearchNow(input = null) {
  // 只有按 Enter 或按「搜尋」按鈕時，才真正搜尋。
  if (input) searchDraftKeyword = input.value;
  searchKeyword = searchDraftKeyword;
  render();
}
...
function renderHome() {
  ...
  return `
    ...
    <section class="toolbar">
      <input class="form-control" value="${escapeHtml(searchDraftKeyword)}" placeholder="輸入完成後按 Enter 搜尋..." oncompositionstart="handleSearchCompositionStart()" oncompositionend="handleSearchCompositionEnd(this)" oninput="handleSearchInput(this)" onkeydown="if(event.key==='Enter'){ event.preventDefault(); runSearchNow(this); }" />
      <button class="btn secondary" type="button" onclick="runSearchNow(document.querySelector('.toolbar input'))">搜尋</button>
      <button class="btn ghost" type="button" onclick="clearSearch()">清除</button>
