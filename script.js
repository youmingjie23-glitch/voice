...
function buildTwoFactorChallenge(title) {
  ...
  const tiles = shuffleList(politicianCaptchaPool).map(person => ({
    ...person,
    target: person.name === target.name,
    selected: false
  }));
  ...
  document.getElementById("twoFactorHint").textContent = `請選出「${target.name}」的圖片。選對才能完成雙重認證。`;
  renderTwoFactorGrid();
}
  ...
  return `
    <section class="card">
      <div class="profile-head">
        <div class="avatar avatar-large">${renderAvatarContent(user.avatar || "🎤")}</div>
        <div>
      ...
      <form class="profile-editor" onsubmit="updateProfile(event)">
        ...
        <div class="avatar-preview-row">
          <div class="avatar avatar-preview" id="avatarPreview">${renderAvatarContent(user.avatar || "🎤")}</div>
          <div class="form-group avatar-url-group">
            <label>自訂頭像圖片網址，可空白</label>
            <input class="form-control" id="profileAvatarUrl" value="${isImageUrl(user.avatar) ? escapeHtml(user.avatar) : ""}" placeholder="例如：https://example.com/avatar.png" oninput="previewAvatar()" />
          </div>
        </div>
        <label class="mini-label">或選擇一個 emoji 頭像</label>
        <div class="avatar-options">
          ${avatarOptions.map(icon => `
            <label class="avatar-option ${user.avatar === icon ? "selected" : ""}" onclick="setAvatarEmoji('${icon}')">
              <input type="radio" name="avatarChoice" value="${icon}" ${user.avatar === icon || (!isImageUrl(user.avatar) && !user.avatar && icon === "🎤") ? "checked" : ""} />
              <span>${icon}</span>
            </label>
          `).join("")}
        </div>
